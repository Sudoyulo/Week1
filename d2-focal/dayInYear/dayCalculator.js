const calculateDayInYear = function(date) {
  ...
  //console.log("input results", year, month, day); 1990 3 2

  const validMonth = function(month) {
    return month && month >= 1 && month <= 12; // between 1-12 <  not <=
  };

  const validDay = function(month, day) {
    return day && day >= 1 && day <= DAYS_IN_MONTH[month - 1]; // <= not <
  };

  const calculateDayNumber = function(month, day) {
    let dayOfYear = day; // day not 1

    for (let i = 1; i < month; i++) {
      dayOfYear += DAYS_IN_MONTH[i - 1]; // month -1 not i
    }

    return dayOfYear; //th day of year 1900
  };
...
/*
    ...
    They run the function with a predetermined parameter and compare the
    output to the value we are expecting to get!
